...
const PseudoLine: React.FC<{ pl: PseudocodeLine; highlighted: boolean }> = ({ pl, highlighted }) => {
  ...
  return (
    <div
      className={`flex items-start gap-1 px-1 py-0.5 rounded transition-all duration-150 ${
        highlighted
          ? 'bg-yellow-50 border-l-2 border-yellow-400 text-yellow-700'
    ...
    >
      ...
      <span style={{ paddingLeft: `${pl.indent * 12}px` }} className="flex-1">
        <span>{pl.text}</span>
        {pl.comment && (
          <span className={`ml-2 ${highlighted ? 'text-yellow-600' : 'text-gray-400'}`}>
            {pl.comment}
          </span>
        )}
      </span>
...
export const AlgorithmExecution: React.FC<Props> = ({
  ...
  }, [similarityConfig, ROWS, COLS, lineMap, activePairIndex]);
  ...
  const skipToEnd = () => {
    ...
    setEditScriptStep(editOps.length - 1);
    setComplete(true);
    setIsPlaying(false);
    ...
    <div className="animate-fade-in flex flex-col h-full">
      ...
      <div className="glass-card px-4 py-2 mb-3 flex items-center gap-3 shrink-0 flex-wrap">
        {countryPairs.length > 1 && (
          <div className="min-w-0 max-w-full overflow-x-auto whitespace-nowrap flex items-center gap-1.5 mr-2">
            <span className="text-xs text-gray-500 shrink-0">
              {comparisonMode === 'pair' ? 'Pair:' : 'Comparison:'}
            </span>
                    ...
                    <tr key={i}>
                      ...
                      {tree2Labels.map((_, j) => {
                        ...
                        return (
                          <td
                            key={j}
                            className={`w-9 h-7 text-center text-[10px] font-mono border border-gray-200 transition-all ${
                ...
                {phase === 'editscript' ? (
                  <div className="space-y-1">
                    {editOps.slice(0, editScriptStep + 1).map((op, i) => (
                      <div
                        key={i}
                        className={`flex items-start gap-1.5 px-1.5 py-1 rounded ${i === editScriptStep ? 'bg-primary-50 border border-primary-200' : ''}`}
                      >
                        <span
                          className={`text-[8px] px-1 py-0.5 rounded font-bold uppercase shrink-0 ${
                  ...
                  <div className="space-y-0.5">
                    {callStack.length === 0
                      ...
                      : callStack.map((e, i) => (
                          <div key={i} className={i === callStack.length - 1 ? 'text-primary-600 font-semibold' : 'text-gray-500'}>
                            {e}
                          </div>
                        ))}
                  </div>
