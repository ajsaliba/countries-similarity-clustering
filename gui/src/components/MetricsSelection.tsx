...
function getTreeFields(tree: TreeNode): Map<string, { label: string; value: string | undefined }[]> {
  ...
  for (const catNode of tree.children) {
    ...
    if (leaves.length > 0) {
      map.set(catNode.label, leaves.map(n => ({ label: n.label, value: n.value })));
    }
  }
  return map;
}
...
export const MetricsSelection: React.FC<MetricsSelectionProps> = ({
  ...
  const getCountryName = (code: string) =>
    countries.find(c => c.code === code)?.name ?? selectedCountries.find(c => c.code === code)?.name ?? code;

  // On mount: generate pairs then auto-select all common fields from loaded trees
  useEffect(() => {
    const pairs = countryPairs.length === 0 ? onGeneratePairs() : countryPairs;
    if (!hasData) return;
    pairs.forEach(pair => {
      if (pair.selectedMetrics.length === 0) {
        ...
        if (t1 && t2) {
          ...
          for (const fields of getCommonFields(t1, t2).values()) allCommon.push(...fields);
          if (allCommon.length > 0) onUpdatePairMetrics(pair.country1, pair.country2, allCommon);
        }
      }
    });
    // Expand all categories on first load
    if (pairs.length > 0) {
      const t1 = loadedTrees[pairs[0].country1];
      const t2 = loadedTrees[pairs[0].country2];
      if (t1 && t2) setExpandedCategories(new Set(getCommonFields(t1, t2).keys()));
    }
  ...
  const toggleField = (field: string) => {
    ...
    onUpdatePairMetrics(
      activePair.country1, activePair.country2,
  ...
  const allPairsHaveFields = countryPairs.length > 0 && countryPairs.every(p => p.selectedMetrics.length > 0);
    ...
    <div className="animate-fade-in flex flex-col h-full">
      <div className="text-center mb-4 shrink-0">
        ...
        <p className="text-gray-500 text-sm">
          Choose which indicators to include for each country pair.
          Fields are read directly from the loaded {Object.keys(loadedTrees).length > 0 ? 'data' : ''} files.
        </p>
      </div>
                ...
                >
                  ...
                  <div className="mt-1">
                    <span className={`text-[10px] px-1.5 py-0.5 rounded ${
                      pair.selectedMetrics.length > 0 ? 'bg-accent-50 text-accent-600' : 'bg-yellow-50 text-yellow-600'
                    }`}>
              ...
              <>
                ...
                <div className="flex items-center justify-between mb-3 pb-3 border-b border-gray-200 shrink-0">
                  ...
                  <div className="flex items-center gap-2">
                    <button onClick={selectAll} className="text-xs text-primary-600 hover:text-primary-700 px-2 py-1 rounded border border-primary-200 hover:border-primary-300 transition-colors">
                      Select all
                    </button>
                    <button onClick={clearAll} className="text-xs text-gray-500 hover:text-gray-600 px-2 py-1 rounded border border-gray-200 hover:border-gray-300 transition-colors">
                      Clear
                    </button>
                  </div>
                </div>
                    ...
                    return (
                      <div key={cat} className="border border-gray-200 rounded-lg overflow-hidden">
                        ...
                        >
                          ...
                          <span className="text-[10px] text-gray-500 ml-auto">{selCount}/{fields.length}</span>
                          <button
                            onClick={e => { e.stopPropagation(); toggleCategoryAll(cat, fields); }}
                            ...
                            {fields.map(field => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  {/* Values */}
                                  <span className="w-28 text-[10px] text-gray-400 font-mono text-right truncate">{v1}</span>
                                  <span className="w-28 text-[10px] text-gray-400 font-mono text-right truncate">{v2}</span>
                                </button>
                              );
                            })}
                          </div>
