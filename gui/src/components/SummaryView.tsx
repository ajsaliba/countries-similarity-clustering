...
import { AlgorithmConfig, Country, CountryPair, SimilarityConfig, TreeNode } from '../types';
...
export const SummaryView: React.FC<SummaryViewProps> = ({
  ...
  // Compute real similarity results for each pair, filtered to selected metrics
  const pairResults = useMemo(() =>
    countryPairs.map(pair => {
      const t1 = loadedTrees[pair.country1];
      const t2 = loadedTrees[pair.country2];
      if (!t1 || !t2) return null;
      const ft1 = pair.selectedMetrics.length ? filterTreeByMetrics(t1, pair.selectedMetrics) : t1;
      const ft2 = pair.selectedMetrics.length ? filterTreeByMetrics(t2, pair.selectedMetrics) : t2;
  ...
  return (
    <div className="animate-fade-in flex flex-col h-full">
      ...
      <div className="flex-1 overflow-auto">
          ...
          <div className="glass-card p-5">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-2xl font-bold text-gray-900">{selectedCountries.length}</div>
              </div>
            </div>
          </div>
          ...
          {showComplexity && selectedAlgorithm && (
            <div className="mt-4 space-y-4">
              <div className="bg-gray-50 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-primary-600 mb-3">{selectedAlgorithm.name}</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white rounded-lg p-4 border border-gray-200">
                    <div className="flex items-center gap-2 mb-2">
                      <Cpu size={14} className="text-yellow-600" />
                      <h5 className="text-sm font-semibold text-yellow-600">Time Complexity</h5>
                    </div>
                    <div className="text-xl font-mono text-gray-900 mb-2">{selectedAlgorithm.timeComplexity}</div>
                  ...
                  <div className="bg-white rounded-lg p-4 border border-gray-200">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <h5 className="text-sm font-semibold text-cyan-600">Space Complexity</h5>
                    </div>
                    <div className="text-xl font-mono text-gray-900 mb-2">{selectedAlgorithm.spaceComplexity}</div>
                    <p className="text-xs text-gray-500 leading-relaxed">
                      {selectedAlgorithm.type === 'chawathe'
                        ? 'Storage for the matching table between all node pairs, plus the edit script operations list.'
                        : 'The TED matrix stores distances between all subtree pairs. Additional space is needed for the forest distance tables and backtrack information.'}
                    </p>
                  </div>
                </div>

                <div className="mt-4">
                  <h5 className="text-sm font-semibold text-gray-700 mb-2">Step-by-step Complexity</h5>
                    ...
                    <tbody>
                      {getStepComplexities(selectedAlgorithm.type).map((step, i) => (
                        <tr key={i} className="border-b border-gray-100">
                          ...
                          <td className="py-2 font-mono text-yellow-600 text-xs">{step.time}</td>
                          <td className="py-2 font-mono text-cyan-600 text-xs">{step.space}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                <h4 className="text-sm font-semibold text-gray-700 mb-3">Practical Performance Notes</h4>
                <ul className="space-y-2 text-xs text-gray-500">
                  <li className="flex items-start gap-2">
                    <Info size={12} className="text-primary-600 mt-0.5 shrink-0" />
                    <span>
                      Data files typically produce trees with 100–200 nodes (10 categories × 10–20 fields each).
                      At this scale, both algorithms run in well under 1 second per pair.
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <Info size={12} className="text-primary-600 mt-0.5 shrink-0" />
                    <span>
                      The bottleneck is typically data collection (network I/O), not the TED computation itself.
                    </span>
                  </li>
                  <li className="flex items-start gap-2">
                    <Info size={12} className="text-primary-600 mt-0.5 shrink-0" />
                    <span>
                      For {countryPairs.length} pair{countryPairs.length !== 1 ? 's' : ''}, the total TED computations
                      scale as O(k²) where k = {selectedCountries.length} countries selected.
                    </span>
                  </li>
                </ul>
              </div>
        ...
        {/* Pairs results table */}
        <div className="glass-card p-5 mb-6">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">Comparison Results per Pair</h3>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-300">
              ...
              {countryPairs.map((pair, i) => {
                ...
                return (
                  <tr key={i} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-2.5 text-center text-gray-500">{pair.selectedMetrics.length}</td>
                    ...
                    <td className="py-2.5 text-center">
                      {simPct !== null ? (
                        <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                          simPct >= 70 ? 'bg-green-50 text-green-600'
                          : simPct >= 40 ? 'bg-yellow-50 text-yellow-600'
